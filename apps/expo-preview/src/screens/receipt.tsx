    ...
    <View style={styles.screen} testID="screen-receipt">
      ...
      >
        <View style={styles.header}>
          ...
          <Text selectable accessibilityRole="header" style={styles.title}>{"Request sent"}</Text>
          <Text selectable style={styles.body}>{"Confirm completion and reference"}</Text>
        </View>
        <View style={styles.content}>
          <View testID="node-receipt.summary" style={nodeStyle({"compactMode":"leaf","regularMode":"leaf","axis":"vertical","width":"fill","height":"hug","align":"stretch","justify":"start","overflow":"visible","columns":2,"splitRatio":0.5,"gap":16,"padding":20})}><View accessible accessibilityLabel="Payment request sent" accessibilityRole="summary" style={styles.card}><Text selectable style={styles.sectionTitle}>{"Payment request sent"}</Text><Text selectable style={styles.heroValue}>{String(data.amount ?? "")}</Text><Text selectable style={styles.body}>{String(data.reference ?? "")}</Text></View></View>
          {(compact ? false : true) ? (<View testID="node-receipt.confirm" style={nodeStyle({"compactMode":"leaf","regularMode":"leaf","axis":"vertical","width":"fill","height":"hug","align":"stretch","justify":"start","overflow":"visible","columns":2,"splitRatio":0.5,"gap":16,"padding":20})}><Pressable testID="action-receipt.confirm" accessible accessibilityLabel="Done" accessibilityRole="button" onPress={() => { events.onDone(); }} style={({ pressed }) => [styles.primaryAction, pressed && styles.pressed]}><Text selectable style={styles.primaryActionLabel}>{"Done"}</Text></Pressable></View>) : null}
        </View>
      </ScrollView>
