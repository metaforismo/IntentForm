      ...
      <section className="relative grid min-w-0 grid-rows-[48px_1fr_34px] bg-[#d9ddda]">
        <div className="relative flex items-center justify-between border-b border-[#bfc5c1] bg-[#f5f6f4]/95 px-3 text-[#343936] shadow-[0_1px_0_rgba(255,255,255,.8)]">
          ...
          <div className="absolute left-1/2 flex -translate-x-1/2 items-center gap-0.5 rounded-lg border border-[#cbd0cc] bg-white p-1 shadow-[0_8px_22px_-18px_rgba(22,30,26,.7)]">
            {([
              ...
              return <button key={item.id} type="button" aria-label={item.label} onClick={() => setTool(item.id)} className={`grid size-7 place-items-center rounded-md ${tool === item.id ? "bg-[#397461] text-white" : "text-[#69706c] hover:bg-[#edf0ed]"}`}><Icon size={14} weight={tool === item.id ? "fill" : "regular"} /></button>;
            })}
        ...
        >
          <div className="mx-auto w-fit origin-top transition-transform duration-300" style={{ transform: `scale(${canvasScale})` }}>
            <div className="mb-2 flex items-center justify-between px-1 text-[9px] font-medium text-[#6f7772]"><span>{screen.title}</span><span className="font-mono">iPhone compact · 375 × 667</span></div>
            <div className="relative flex h-[667px] w-[375px] flex-col overflow-hidden rounded-[42px] border-[9px] border-[#202421] bg-[#fbfcf9] px-7 pb-7 pt-5 text-[#181c1a] shadow-[0_38px_80px_-34px_rgba(25,35,30,.5),inset_0_0_0_1px_rgba(255,255,255,.22)]">
              ...
              <div className="flex min-h-0 flex-1 flex-col gap-4">
                {screen.nodes.map((node) => {
                  ...
                  const persistent = node.kind === "primary-action" && node.layout.placement?.compact === "persistent-bottom";
                  return (
                    <motion.div
                      layout
                      ...
                      onDragEnd={(_, info) => {
                        ...
                        if (info.offset.y > 28 && node.layout.placement?.compact !== "persistent-bottom") {
                          onSelectNode(node.id);
                          updateNodeById(node.id, (draft) => { if (draft.layout.placement) draft.layout.placement.compact = "persistent-bottom"; }, "Anchored primary action to the bottom safe area on compact screens.");
                        } else if (info.offset.y < -28 && node.layout.placement?.compact === "persistent-bottom") {
                          onSelectNode(node.id);
                          updateNodeById(node.id, (draft) => { if (draft.layout.placement) draft.layout.placement.compact = "inline"; }, "Returned primary action to the semantic stack on compact screens.");
                        }
                      }}
                      key={node.id}
                    ...
                    >
                      ...
                      {selected && persistent ? <span className="pointer-events-none absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full border border-[#85b4ee] bg-[#edf5ff] px-2 py-1 font-mono text-[8px] text-[#1769ce]">Bottom safe area · semantic</span> : null}
                    </motion.div>
                  );
                })}
              </div>
        ...
        <div className="flex items-center justify-between border-t border-[#bfc5c1] bg-[#eef0ee] px-3 text-[9px] text-[#6f7772]">
          <span className="font-mono">{tool === "select" ? "Drag an action vertically to change its semantic placement" : `${tool[0]?.toUpperCase()}${tool.slice(1)} tool`}</span>
          <div className="flex items-center gap-1 rounded-md border border-[#c8ceca] bg-white p-0.5">
            <button type="button" aria-label="Zoom out" onClick={() => setZoom((value) => Math.max(60, value - 10))} className="grid size-5 place-items-center rounded text-[#707873] hover:bg-[#edf0ed]"><Minus size={10} /></button>
            <span className="w-9 text-center font-mono">{zoom}%</span>
            <button type="button" aria-label="Zoom in" onClick={() => setZoom((value) => Math.min(120, value + 10))} className="grid size-5 place-items-center rounded text-[#707873] hover:bg-[#edf0ed]"><Plus size={10} /></button>
          </div>
        </div>
      </section>
          ...
          <div data-testid="semantic-inspector" className="divide-y divide-[#343835]">
            ...
            <section className="grid gap-4 p-4">
              ...
              {selectedNode.kind === "primary-action" && selectedNode.layout.placement ? (
                <SegmentedControl label="Compact placement" value={selectedNode.layout.placement.compact} options={[{ value: "inline", label: "Inline" }, { value: "persistent-bottom", label: "Bottom safe area" }]} onChange={(value) => updateNode((node) => { if (node.layout.placement) node.layout.placement.compact = value; }, value === "persistent-bottom" ? "Anchored action to the bottom safe area." : "Returned action to the semantic stack.")} />
              ) : null}
            </section>
