...
export function VerifyStage({
  ...
  const selectedNavigationTarget = selected
    ...
    : null;
  const evidenceIsCurrent = buildEvidenceState === "passed";
        ...
        <section className="min-h-0 overflow-auto bg-[var(--canvas)] p-5" aria-label="Selected verification evidence">
          {selected ? (
            <div className="mx-auto max-w-4xl">
              <div className="flex items-start justify-between gap-4 border-b border-[var(--line)] pb-4"><div><div className="flex items-center gap-2"><SeverityIcon severity={selected.severity} /><span className="font-mono text-[9px] uppercase text-[var(--faint)]">{selected.severity} · {selected.status}</span></div><h3 className="mt-1.5 max-w-3xl text-[13.5px] font-semibold leading-snug tracking-[-.01em] text-[var(--ink)]">{selected.violatedIntent}</h3></div><button type="button" onClick={() => inspectFinding(selected)} disabled={!selectedNavigationTarget} title={selectedNavigationTarget ? "Open the exact evidence target" : "This finding has no current canvas target"} className="inline-flex min-h-8 shrink-0 items-center gap-1 rounded border border-[var(--line)] bg-[var(--panel)] px-3 text-[10px] font-semibold text-[var(--muted)] hover:text-[var(--ink)] disabled:cursor-not-allowed disabled:opacity-40"><ArrowSquareOut size={11} /> Show on canvas</button></div>
              ...
              <section className="mt-5 rounded border border-[var(--line)] bg-[var(--panel)] p-4">
                ...
                {repairPreview?.findingId === selected.id ? (
                  <div className="mt-4 border-t border-[var(--line)] pt-4" aria-label="Repair preview">
                    ...
                    {repairPreview.sourceFingerprint !== sourceFingerprint ? <p role="alert" className="mt-3 rounded border border-[var(--warn)]/25 bg-[var(--warn-soft)] p-2.5 text-[9px] leading-relaxed text-[var(--warn)]">This repair preview is stale because the graph changed. Dismiss it, re-run verification, and create a new fingerprint-bound preview.</p> : null}
