...
export function AppearanceInspector({ graph, nodes, onUpdate }: { graph: SemanticInterfaceGraph; nodes: readonly SemanticNode[]; onUpdate(mutate: (node: SemanticNode) => void, notice: string): void }) {
  ...
  return <>
    ...
    <DisclosureSection title={`Selection colors · ${colors.length}`} defaultOpen={false}>
      {colors.length === 0 ? <p className="text-[10.5px] leading-4 text-[var(--if-text-tertiary)]">No authored fill or stroke colors in this selection.</p> : colors.map((color) => <div key={`${color.token ?? "literal"}:${color.color}`} className="grid grid-cols-[18px_minmax(0,1fr)_auto] items-center gap-2"><span className="size-4 rounded-[4px] border border-[var(--if-border)]" style={{ background: color.color }} /><span className="min-w-0 truncate font-mono text-[9.5px]">{color.token ?? color.color}</span><span className="text-[9px] text-[var(--if-text-tertiary)]">{color.usages} · {color.origins.join("+")}</span></div>)}
