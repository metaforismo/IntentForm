    ...
    <>
      ...
      {open ? createPortal(
        <div className="fixed inset-0 z-[80] grid place-items-center bg-[rgba(9,20,17,.34)] p-3 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
          <section ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="desktop-control-title" className="max-h-[min(760px,calc(100dvh-24px))] w-full max-w-3xl overflow-auto rounded-[8px] border border-[var(--line)] bg-[var(--surface-strong)] shadow-[0_30px_90px_-45px_rgba(0,0,0,.7)]">
            ...
            <div className="grid gap-5 p-5 lg:grid-cols-2">
              <div className="space-y-5">
                <section className="rounded-[6px] border border-[var(--line)] bg-[var(--surface)] p-4">
                  ...
                  <button type="button" disabled={pending} onClick={() => perform(() => api.chooseProject())} className="mt-4 inline-flex min-h-9 items-center gap-2 rounded-[5px] border border-[var(--line)] px-3 text-[10px] font-semibold hover:bg-[var(--hover)] disabled:opacity-50">Choose another project</button>
                </section>
                ...
                <section className="rounded-[6px] border border-[var(--line)] bg-[var(--surface)] p-4">
                  ...
                  {mcp.phase === "ready" ? <button type="button" disabled={pending} onClick={() => perform(() => api.copyMcpConfiguration())} className="mt-2 inline-flex min-h-9 items-center gap-2 rounded-[5px] bg-[var(--accent-deep)] px-3 text-[10px] font-semibold text-white disabled:opacity-50"><ClipboardText size={13} /> Copy MCP client configuration</button> : null}
                </section>
              </div>
              ...
              <div className="space-y-5">
                <section className="rounded-[6px] border border-[var(--line)] bg-[var(--surface)] p-4">
                  ...
                  <div className="mt-3 grid grid-cols-2 gap-2">
                    {snapshot.toolchains.map((item) => <div key={item.id} title={item.detail} className="rounded-[5px] border border-[var(--line)] bg-[var(--field)] p-2.5"><span className="flex items-center gap-2 text-[10px] font-semibold"><span className={`size-1.5 rounded-full ${statusColor(item.status)}`} />{item.label}</span><span className="mt-1 block truncate font-mono text-[8px] text-[var(--muted)]">{item.version ?? item.status}</span></div>)}
                  </div>
                </section>
                ...
                <section className="rounded-[6px] border border-[var(--line)] bg-[var(--surface)] p-4">
                  ...
                  {snapshot.update.supported ? <button type="button" disabled={pending} onClick={() => perform(() => api.checkForUpdates())} className="mt-3 min-h-9 rounded-[5px] border border-[var(--line)] px-3 text-[10px] font-semibold hover:bg-[var(--hover)] disabled:opacity-50">Check signed feed</button> : null}
                </section>
              </div>
            </div>
            {error ? <div role="alert" className="mx-5 mb-5 flex items-start gap-2 rounded-[5px] border border-red-300/40 bg-red-50 p-3 text-[10px] text-red-900"><Warning size={13} weight="fill" className="mt-0.5" />{error}</div> : null}
          </section>
        </div>,
        document.body,
      ) : null}
