    ...
    <>
      ...
      {open ? createPortal(
        <div className="fixed inset-0 z-[70] grid place-items-center bg-black/35 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
          <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="ecosystem-title" tabIndex={-1} className="max-h-[min(720px,calc(100dvh-32px))] w-full max-w-2xl overflow-auto rounded-[24px] border border-[var(--line)] bg-[var(--surface)] p-5 text-[var(--ink)] shadow-2xl sm:p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <button ref={closeRef} type="button" aria-label="Close ecosystem status" onClick={() => setOpen(false)} className="grid size-9 shrink-0 place-items-center rounded-xl text-[var(--muted)] hover:bg-[var(--hover)] hover:text-[var(--ink)]"><X size={16} /></button>
            </div>

            {loading ? <div role="status" className="mt-6 rounded-2xl border border-[var(--line)] bg-[var(--chip)] p-5 text-sm text-[var(--muted)]">Reading local integrity state…</div> : null}
            {error ? <div role="alert" className="mt-6 rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-950">{error}</div> : null}
            {status && !loading ? (
              <div className="mt-6 grid gap-4">
                <div className="grid gap-3 sm:grid-cols-3">
                  <article className="rounded-2xl border border-[var(--line)] bg-[var(--chip)] p-4"><ShieldCheck size={18} className="text-[var(--accent)]" /><strong className="mt-3 block text-sm">{status.trust.activeKeyCount} active trust roots</strong><span className="mt-1 block text-xs leading-relaxed text-[var(--muted)]">Unknown and revoked publishers fail closed.</span></article>
                  <article className="rounded-2xl border border-[var(--line)] bg-[var(--chip)] p-4"><Cube size={18} className="text-[var(--accent)]" /><strong className="mt-3 block text-sm">{status.packages.length} locked packages</strong><span className="mt-1 block text-xs leading-relaxed text-[var(--muted)]">Compilers use vendored graph data and never fetch.</span></article>
                  <article className="rounded-2xl border border-[var(--line)] bg-[var(--chip)] p-4"><CloudSlash size={18} className="text-[var(--accent)]" /><strong className="mt-3 block text-sm">Sync {status.sync.mode}</strong><span className="mt-1 block text-xs leading-relaxed text-[var(--muted)]">{status.sync.keyOwnership}; {status.sync.retentionDays}-day policy.</span></article>
                </div>
                <section aria-labelledby="installed-packages-title" className="rounded-2xl border border-[var(--line)] p-4">
                  ...
                  {status.packages.length === 0 ? <p className="mt-3 text-xs leading-relaxed text-[var(--muted)]">No external package is installed. The built-in component and token libraries remain fully available offline.</p> : (
                    <ul className="mt-3 grid gap-2">
                      {status.packages.map((entry) => <li key={entry.id} className="flex flex-wrap items-center justify-between gap-2 rounded-xl bg-[var(--chip)] px-3 py-2 text-xs"><span><strong>{entry.id}</strong> <span className="font-mono text-[var(--muted)]">{entry.version}</span></span><span className={entry.cache === "verified" ? "text-[var(--accent)]" : "text-[var(--danger)]"}>{entry.cache}</span></li>)}
                    </ul>
                  )}
                </section>
